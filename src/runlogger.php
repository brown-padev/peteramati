...
class RunLogger {
    ...
    /** @return string */
    function log_dir() {
        $logroot = $this->pset->conf->log_dir();
        return "{$logroot}/run{$this->repo->cacheid}.pset{$this->pset->id}";
    }
    ...
    function log_file($suffix) {
        ...
        return "{$logroot}/run{$this->repo->cacheid}.pset{$this->pset->id}/repo{$id}.pset{$this->pset->id}{$suffix}";
    }
